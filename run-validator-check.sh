...
SPEC="$REPO/src/sdk/addons/zz-external-groceryflywheel.test.ts"
cat > "$SPEC" <<EOS
import { describe, expect, it } from "vitest";
import { readFileSync } from "node:fs";
import { validateAddOnManifest } from "./validation";
const manifest = JSON.parse(readFileSync("$HERE/addon.json", "utf8"));
describe("external addon.grocery-flywheel vs real validator", () => {
...
EOS
trap 'rm -f "$SPEC"' EXIT
cd "$REPO" && npx vitest run src/sdk/addons/zz-external-groceryflywheel.test.ts
